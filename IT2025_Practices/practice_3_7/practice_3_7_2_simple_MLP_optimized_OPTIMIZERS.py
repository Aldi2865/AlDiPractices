...
from tensorflow.keras.optimizers import SGD # type: ignore # Додано для використання Adam оптимізатора
from tensorflow.keras.optimizers import RMSprop # type: ignore # Додано для використання Adam оптимізатора
from tensorflow.keras.callbacks import EarlyStopping # type: ignore # Додано для ранньої зупинки навчання
from tensorflow.keraßs.layers import BatchNormalization # type: ignore # Додано для нормалізації пакетів
...
# Оптимізатор: SGD (Стохастичний Градієнтний Спуск)
# learning_rate: Швидкість навчання. Для SGD її часто потрібно підбирати ретельніше, ніж для Adam.
#                Поширені значення: 0.1, 0.01, 0.001. Почнемо з 0.01.
# momentum: Момент. Допомагає прискорити SGD у правильному напрямку та зменшити коливання.
#           Значення 0.9 є дуже поширеним та часто ефективним.
# nesterov: Використання моменту Нестерова. Зазвичай дає невелике покращення порівняно
#           зі стандартним моментом, якщо momentum > 0.
# optimizer = SGD(learning_rate=0.001, momentum=0.9, nesterov=True)


# Оптимізатор: RMSprop
# RMSprop - це також адаптивний оптимізатор, як і Adam.
# learning_rate: Швидкість навчання. Стандартне значення 0.001 часто працює добре.
# rho: Фактор згасання для ковзного середнього квадратів градієнтів. Стандартне значення 0.9.
# momentum: Як і в SGD, до RMSprop можна додати моментум.
# centered: Якщо True, нормалізує градієнт за допомогою оцінки його дисперсії.
optimizer = RMSprop(learning_rate=0.001, rho=0.9, momentum=0.0, centered=False)

# Альтернатива: RMSprop з моментумом (іноді працює краще)
# optimizer_rmsprop = RMSprop(learning_rate=0.001, rho=0.9, momentum=0.9)
